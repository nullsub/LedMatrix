...
static void show_volume(pa_stream *s, int success, void *data)
{
    ...
    if (!volume->mute)
        len = snprintf(display, BAR_COUNT - 1, "Volume: %d%%", scaled);
    else
        len = snprintf(display, BAR_COUNT - 1, "Muted");

    memset(display+len, ' ', BAR_COUNT-len);

    	imon_write(display);
	printf("imon_write: show_volume()\n");
